jQuery(document).ready(function ($) {
	posts_slick_options.forEach((element, index) => {
		...
		let dirAttr = (rtl) ? 'rtl' : '';
		let arrows = (element.arrows!=null)
			? element.arrows
			: false;
		...
		let centerMode = (element.centerMode!=null)
			? element.centerMode
			: false;
		...
		$(window).on("load", function () {
			if (typeof jQuery(".posts-custom-slider-"+index).slick == "function") {
				$(".posts-custom-slider-"+index).attr("dir", dirAttr).slick({
					autoplay: autoplay,
